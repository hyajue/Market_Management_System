...
public class UpdateUser extends JDialog implements ActionListener {
  ...
  public UpdateUser(Frame frame, String title, boolean modal) {
  ...
  void jButton1_actionPerformed(ActionEvent e) {
    ...
    if(userPassword.equals(testPassword)){
      try{
        ...
        sql1.executeUpdate("UPDATE [UML_CS].[dbo].[user] SET userAge = '"+userAge+"',userGender = '"+userGender+"', userAuthority = '"+userAuthority+"', userAddress = '"+userAddress+"' , userPassword = '"+userPassword+"' WHERE userName = '"+userName+"'") ;
        JOptionPane.showMessageDialog(null," 修改成功","",JOptionPane.DEFAULT_OPTION);
        cancel();
      }
